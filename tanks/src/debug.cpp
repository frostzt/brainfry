...
// Ok so here's what going on here since I have `-Wformat-nonliteral` enabled the `const char* fmt` is causing issues
// we'll use `__format__` to make sure that the `const char *fmt` is being treated as `__printf__`
// @see https://stackoverflow.com/questions/36120717/correcting-format-string-is-not-a-string-literal-warning
